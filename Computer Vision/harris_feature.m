...
im=imread('checkerboard.jpg'); % Step 0: Read Image
figure ;imshow(im);
im1=im2double(im);
figure ;imshow(im1);
dx = [-1 -2 0 1 2]; % image derivatives
dy = dx';
...
disp(t);
%% Part 3 - Select for E and R the 48 most salient points
% Get the coordinates with maximum cornerness responses
% Write a function to obtain the 48 most salient points
numPts = 48;
[sortR,RIX] = sort(R(:),'descend');
[a, b] = ind2sub([r c],RIX);%The mapping from linear indexes to subscript equivalents for the matrix
figure; imshow(im1, []); hold on; xlabel('Max 48 points');% Get the coordinates with maximum cornerness responses     
for i=1:48
	plot(a(i), b(i), 'r+'); 
end  
...
figure; imshow(im1, []); hold on; xlabel('Max 48 points'); %labeling along with X axis    
for i=1:48 
	plot(a(i), b(i), 'r+'); 
end